<template>
  <div class="btn-group btn-group-sm">
    <button class="btn btn-default" :title="show_row"
      :class="{ '-nested-comp-open-btn': isDisplayRowVisible }"
      @click="toggleNestedComp('DisplayRow')">
      <i class="fa fa-list-ul"></i>
    </button>
    <button class="btn btn-default" :title="del_row"
            @click="delrow">
      <i class="fa fa-trash"></i>
    </button>
  </div>
</template>
<script>

export default {
  props: ['row', 'nested'],
    data: () => ({
        show_row:"",
        del_row:""
    }),
  mounted () {
      this.show_row = (this.$parent.$parent.$parent.$parent.$parent.app_local == 'en'?'Display row':'عرض الصف');
      this.del_row = (this.$parent.$parent.$parent.$parent.$parent.app_local == 'en'?'Delete row':'حذف الصف');
    $(this.$el).find('button[title]').tooltip()
  },
  computed: {
    isDisplayRowVisible () {
      if (this.nested.comp !== 'DisplayRow') return
      return this.nested.visible
    },
    isFriendsTableVisible () {
      if (this.nested.comp !== 'FriendsTable') return
      return this.nested.visible
    }
  },
  methods: {
    toggleNestedComp (comp) {
      const { nested } = this
      if (nested.comp === comp) return nested.$toggle()
      nested.$toggle(comp, true)
    },
    delrow () {
        this.$root.$emit('DelRow', this.row);
    }
  }
}
</script>
<style>
.-nested-comp-open-btn {
  color: #fff !important;
  background-color: #337ab7 !important;
}
</style>
