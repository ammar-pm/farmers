...
class ChartJSFileController extends Controller
{
    ...
    public function chartjs(Request $request)
    {
        ...
        if (isset($id)) {
            ...
            $res = ChartJSFileController::get_chartjs_payload($id,$file->file_data_text,null,$fdim,$fagg,$fbase,$charttype,$lang,$alertlang,$years_fields);
            return $res;
        }
    }

    public function get_chartjs_payload($file_id,$file_data,$scale,$fdim,$fagg,$fbase,$charttype,$lang,$alertlang,$years_fields)
    {
        ...
        ///////////////// get all cat headers for base catgeorization abed2222
        ...
        //sleep(2);
        sort($years_fields);
        $data['out_labels'] = (empty($years_fields))?$years : $years_fields;
        ...
        if (empty($totres)) {
            $data['msg'] = [
                'msg_code' => 2,
                'msg_text' => 'Please select new dimensions and base to visulize'
            ];
            return $data;
        }
        /////////////////////// query for pie and doughnut charts /////////////////////////
        ...
        $pie_don_color = array_map(function ($value){
        ...
        foreach($totres as $key=>$totres_val){
            ...
            array_walk($saved_years, function(&$val, $key) use ($values,$yearsaggr) {
                $val = (in_array($val, $yearsaggr))?floatval($values[array_search($val, $yearsaggr)]):null; // 0
            });
            $values = $saved_years;
            ...
            $dataset_data = [
                'label' => $label_agggname,
                'toollabel' => $tmp_agggname,
                'fill' => '',
                'pointStyle' => '',
                'pointRadius' => '',
                'lineTension' => 0,
